...
- **触发**：Zero 按 `J` 键，前方挥砍（扇形/矩形判定，刀光用 `blade.png`）
...
- **时间规则**：
  ...
  - 空中挥砍独立间隔 0.5s（`MELEE_AIR_CD` = 30 帧，跳跃挥砍后需等待，落地后剩余冷却继续生效）
...
- **刀光表现**：blade.png 不转动，仅随挥砍进度渐隐消失（`alpha` 从 1 线性降到 0）；刀光中心向前偏移半个刀光宽，避免压住角色身体
- **方向**：面朝右刀光尖端指向右（blade 逆时针旋转 90°），面朝左镜像
...
- 关卡尺寸：`level.json` 当前网格 63×18 Tile（1764×504 素材像素），**实际砖块只覆盖 col 9~62、row 0~14**（左侧/底部留有空白网格），支持动态扩展（编辑模式下在边界外放置砖块自动扩大地图）
...
- **怪物自由放置**：编辑模式下怪物可放在任意像素位置（中心对齐鼠标，不按格子），放置前检测不与已有怪物、不与地形矩形重叠
- **怪物尺寸**：普通怪高度统一为角色高度（35），宽度按类型（soldier 16 / bee 22 / floater 20）；Boss 50×50（2 倍宽高）
- 摄像机：支持水平+垂直居中跟随；**边界限制以「已放置砖块的包围盒」为准**（`MAP_BOUNDS` 由 `updateMapBounds()` 扫描重算），镜头永不平移到砖块范围之外（地图比视口窄/矮时居中），避免露出无砖块空白区
...
- `enemies[]` — 怪物列表（每只含 `meleeLock`：近战命中冷却帧，>0 时挥砍跳过该怪，仅 Zero 近战使用）
...
MELEE_AIR_CD: 30 (0.5s)     // 空中挥砍独立间隔（跳跃挥砍后需等待，落地后剩余冷却继续生效）
...
16. Zero 挥砍逐帧命中判定（`doMeleeHit`，怪物 `meleeLock` 6 帧冷却 + hitstop 全冻结）+ 更新怪物 AI（`meleeLock` 递减）+ 物理
17. 玩家子弹 vs 怪物/墙碰撞
18. 敌人子弹 vs 玩家碰撞
19. 玩家碰怪物伤害
20. 更新粒子/残影
...
| `meleeRect(stage)` | 计算近战攻击矩形（blade 旋转 90° 后范围） |
| `meleeStageMult(stage)` | 近战三段范围放大倍率 |
| `drawMeleeEffect(p, cx, cy)` | 绘制近战刀光（blade.png，渐隐消失） |
