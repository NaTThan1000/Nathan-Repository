...
- **时间规则**：
  - 段间冷却 0.2s（`MELEE_CD` = 12 帧，攻击频率上限）
  - 连击重置 0.3s（`MELEE_RESET` = 18 帧，超过未攻击则重置回第 1 段）
  - 第三段后硬直 0.3s（`MELEE_FINAL_LOCK` = 18 帧，期间不能攻击）
...
MELEE_CD: 12 (0.2s)         // 段间攻击冷却（攻击频率上限）
MELEE_RESET: 18 (0.3s)      // 连击重置窗口（超过未攻击则重置回第 1 段）
MELEE_FINAL_LOCK: 18 (0.3s) // 第三段后的硬直（期间不能攻击）
```
...
| `doMelee(stage)` | Zero 近战挥砍判定（矩形重叠检测敌人） |
