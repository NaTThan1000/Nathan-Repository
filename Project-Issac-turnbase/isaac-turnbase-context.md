...
> 文件: `Project-Issac-turnbase/isaac-turnbased-demo.html` | 单文件 ~1100 行 | 状态: AP行动点数系统 + 行为队列预览 + 回合制原型可玩
...
### 1.2 核心概念
受《以撒的结合》启发，将 Roguelike 回合制与 AP 行动点数系统结合：
- 玩家在网格化房间内通过消耗 AP 点数规划行动
- **M-AP** (移动点数) 控制移动，**A-AP** (攻击点数) 控制射击
- **预操作机制**：先规划行为序列（幽灵预览 + 地面箭头），按空格确认执行
- 行为队列按顺序执行（移动动画 → 射击动画），全部结束后切换回合
- 玩家回合 → 怪物回合（占位）→ 循环
...
```
player_select ──→ player_shoot_dir ──→ player_select
      │                                      │
      └─── (空格执行) ──→ player_execute ──→ monster_turn ──→ player_select
```

| 阶段 | 说明 |
|------|------|
| `player_select` | 行为选择：WASD移动、J进入射击、L撤销、空格执行 |
| `player_shoot_dir` | 射击方向选择：WASD选方向、K确认、L取消 |
| `player_execute` | 执行阶段：按队列顺序执行动画，锁定输入 |
| `monster_turn` | 怪物回合（占位 600ms），结束后重置 AP |

- 执行阶段通过 `execIndex` 逐个消费 `actionQueue`，每个动作完成后 `isExecIdle()` 检查触发下一动作
...
| 日期 | 更新内容 |
|------|---------|
| 2026-07-13 | **AP行动点数系统全面重构**：实现回合制 AP 核心玩法（M-AP/A-AP 双点数、行为队列预操作、幽灵复制体预览、地面箭头指引）。新增按键映射（WASD移动/J射击/K确认/L撤销/空格执行）、回合循环（玩家→怪物→玩家）。玩家属性系统（HP/移速/射速/攻击/射程/运气）。**智能M-AP管理**：自由移动区按相反方向自动撤销移动并返还 AP，射击设为 checkpoint 锁定之前的移动。**射击预览**从幽灵复制体位置出发。**行为块可视化**：新增 #action-bar 区域，行为块左→右排列，L撤销右→左删除。子弹射程从 3.0 → 6.0。角色渲染提取为 drawCharacterAt() 统一绘制本体和幽灵。更新文档全部章节（核心概念、系统设计、界面交互、游戏流程、架构图、函数索引、数据流）。 |
