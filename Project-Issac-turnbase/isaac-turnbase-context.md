...
> 文件: `Project-Issac-turnbase/isaac-turnbased-demo.html` | 单文件 ~790 行 | 状态: 原型可玩 + Godot 迁移规划
...
- 移动有平滑动画（280 像素/秒），动画期间锁定输入
- 移动受限：网格边界外（墙壁）不可移动
- 朝向跟踪：记录玩家最后移动/射击方向（↑↓←→），影响角色渲染（瞳孔位置、嘴巴）
...
| `drawPlayer()` | 594 | 像素风角色绘制（身体、眼睛、脚、方向三角） |
...
gameLoop (每帧)
  → updateMove(dt) → 更新 player.px/py → 到达后切回 idle
...
| `isaac-asset-desc.md` | 文档 | 资源替换步骤指南（代码→图片替换教程） |
| `godot-setup-checklist.md` | 文档 | Godot 引擎安装与上手清单（含半回合制实现思路） |
| `304_爱给网_aigei_com.png` | 图片 | 参考素材图片 |
