...
| 日期 | 更新内容 |
|------|---------|
| 2026-07-14 | **完整回合制战斗系统**：统一预览→空格执行模型，队列累积所有操作(移动+射击)，Esc倒序撤销。↑↓←→进入射击方向选择。**怪物系统**：裂口尸(HP10)，2/1格交替AI，同时移动+碰撞检测，红色箭头路径预览，平滑动画插值，血条(黑色底槽+红色填充+白色延迟扣除)，受击白闪抖动，伤害飘字(黄色迸发)，血雾粒子，死亡爆浆。**碰撞重写**：怪物→玩家按移动方向击退(失败反弹)；玩家→怪物按移动方向击退(失败反弹)。**无敌系统**：受击后3回合闪烁无敌。**伤害预览**：队列+当前方向累计预伤害白闪血条+致死骷髅。动画速度翻倍(BULLET_SPEED 560/EXEC_MOVE_SPEED 170)。C键生怪，0AP自动结束回合。UI重构(ZCOOL KuaiLe字体/HP心形/AP圆点/操作弹窗/回合确认弹窗)。怪物改用角色精灵渲染。 |
