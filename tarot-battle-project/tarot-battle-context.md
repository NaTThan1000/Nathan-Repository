...
> 参考文件: `tarot-battle-optimizing1.csv`

#### 🔴 严重过强 (需削)

| 牌 | 问题描述 | 修改建议 | 优先级 |
|----|---------|---------|--------|
| 6U 恋人正位 | 抽2大牌选1打出+额外大牌效果，等价于免费顶级大牌 | 改为"抽2大牌选1打出，另1放回牌堆底" | 🔥必须改 |
| 3U 女皇正位 | 无条件+3卡差，纯正面无代价 | 改为"抽2张小牌打出"或"抽3张选2打出、1丢弃" | 🔥必须改 |
| 15U 恶魔正位 | 夺敌方整阵牌，最大可造成40-50点净摆动 | 改为"将敌方小牌与己方小牌互换"而非直接夺取 | 🔥必须改 |
| 17U 星星正位 | 看5选2，牌差+情报差双重碾压 | 改为"看顶3选2打出"或"看5选2但其余放回牌堆" | 🔥必须改 |

#### 🟡 偏强 (可选微调)

| 牌 | 问题描述 | 修改建议 | 优先级 |
|----|---------|---------|--------|
| 12R 倒吊人逆位 | 摧毁敌方整阵太狠 | 改为"选阵敌方一半小牌(向上取整)丢弃" | ⚡强烈建议 |
| 14R 节制逆位 | 选数字全清太精准，实体操作复杂 | 改为"选数字1~10，该数字及±1的牌全弃" | ⚡强烈建议 |

#### 🔵 严重过弱 (需加强)

| 牌 | 问题描述 | 修改建议 | 优先级 |
|----|---------|---------|--------|
| 10U 命运之轮正位 | 纯旋转零收益，几乎等于浪费大牌 | 改为"己方小牌右移后，新进入的阵+3分额外加分" | 🔥必须改 |
| 10R 命运之轮逆位 | 纯旋转零收益，几乎等于浪费大牌 | 改为"己方小牌左移后，新进入的阵+3分额外加分" | 🔥必须改 |
| 1U 魔术师正位 | 仅移动1张牌，影响太小 | 改为"选2阵，第1阵己方所有小牌移至第2阵" | 🔥必须改 |
| 1R 魔术师逆位 | 移动敌方1张牌到另一阵，无战术价值 | 改为"选敌方1阵，该阵所有小牌平均分配到其他3阵" | 🔥必须改 |
| 3R 女皇逆位 | ≤3小牌弃掉太挑条件，经常无事发生 | 改为"选阵双方所有数字牌丢弃(保留宫廷牌)"或扩到≤4 | ⚡强烈建议 |
| 17R 星星逆位 | 互换+送敌方2张=主动帮对手 | 去掉"敌方抽2张"部分，改为纯粹互换 | 🔥必须改 |
| 8R 力量逆位 | 需己方≥3张前提太苛刻 | 降低门槛为"己方至少1张小牌"，或去掉条件 | ⚡强烈建议 |

#### 🟢 设计优化建议

| 项目 | 问题描述 | 建议 | 优先级 |
|------|---------|------|--------|
| 18U 月亮正位 暗置牌 | 暗置牌难以追踪 | 实体牌可翻面放置，设专门背面朝上区防混淆 | 💡可选 |
| 无视元素限制机制 | 8种效果涉及，终局计分时花色与阵不匹配 | 统一改为：终局时按实际所属花色阵计分 | 💡可选 |
| 21U 世界正位 即胜条件 | 理论触发率<2%，几乎永远不能赢 | 放宽为"含任意8个不同数字"即可即胜 | 💡可选 |
| 宫廷牌=10点强度 | 16张宫廷牌占28.5%，每张10点远超平均5.5点 | 考虑降为8点或减少宫廷牌数量 | 💡可选 |
...
├── tarot-battle-optimizing1.csv   ← v4→v5 大牌平衡性优化审查表
...
| 日期 | 分支/提交 | 修改内容 |
|------|-----------|----------|
| 2026-07-13 | `dev/tarot-update` | **大牌平衡性审查 v4→v5**：①新增 `tarot-battle-optimizing1.csv` 优化审查表，覆盖 18 条建议（🔴4条严重过强需削、🟡2条偏强可选微调、🔵8条严重过弱需加强、🟢4条设计优化）；②同步到上下文文档新增 3.3 节"大牌平衡性优化建议"，逐牌列出问题、建议与优先级；③更新项目文件清单 |
