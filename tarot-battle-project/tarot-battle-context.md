...
// 动效队列
_flyAnimations[]               // [{type:'draw'|'play_minor'|'play_major', card, targetPlayer, targetSuit}]
```
...
动效:
  processFlyAnimations / executeFlyAnimation / highlightTargetZone
```
...
| 日期 | 分支/提交 | 修改内容 |
|------|-----------|----------|
| 2026-07-11 | `dev/tarot-update` | **卡牌飞行动效**：①新增 CSS `.card-fly` 飞行卡牌样式（固定定位、金色边框、阴影），大牌变体 `.major-fly`；②`processFlyAnimations()` 动效队列处理函数，双重 RAF 确保 DOM 就绪后执行，多动画 80ms 间隔依次播放；③`executeFlyAnimation()` 创建 overlay 元素从源位置 CSS transition 飞向目标位置（0.52s cubic-bezier 缓动），到达后 `highlightTargetZone()` 目标阵金色光晕闪烁 0.4s；④集成点：`startTurn()` 抽牌动效（牌堆→抽牌区）、`playMinorToElement()` 小牌打出动效（抽牌区→目标阵）、`majorDecision()` 大牌打出动效（抽牌区→目标阵）；⑤修复 `renderDrawArea()` 宫廷牌打出后 `drawnMinor=null` 时落空显示"等待抽牌"的渲染分支缺失 |
